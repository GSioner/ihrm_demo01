<template>
  <div class="job-info">
    <!-- 打印机图标 -->
    <div class="printer">
      <el-tooltip content="岗位信息">
        <router-link :to="`/employees/printer/${staffId}?type=job`">
...
export default {
  ...
  async created() {
    const res = await getStaffJobInfo(this.staffId)
    this.formData = { ...this.formData, ...res }
    this.depts = await getStaffSimpleInfo()
  },
  methods: {
    async saveJob(data) {
      try {
        await editStaffJobInfo(this.staffId, data)
